...
export const Route = createFileRoute("/noticias/$slug")({
  ...
  head: ({ loaderData }) => {
    if (!loaderData) {
      return { meta: [{ title: "Notícia indisponível — Portal Aurora" }, { name: "robots", content: "noindex" }] };
    }
    const { post } = loaderData;
    return {
      meta: [
        { title: `${post.title} — Portal Aurora` },
    ...
    <article>
      <div className="border-b border-border bg-surface">
        <div className="container-page py-12">
          <p className="kicker">{post.categories?.name ?? "Notícia"}</p>
      ...
      {post.cover_url && (
        <div className="container-page -mt-0 py-8">
          <CoverImage
            src={post.cover_url}
            alt={post.title}
            priority
      ...
      {related.length > 0 && (
        <div className="container-page pb-16">
          <h2 className="mb-6 border-b border-border pb-3 text-2xl">Leia também</h2>
